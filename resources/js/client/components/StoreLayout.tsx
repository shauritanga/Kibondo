...
import { formatMoney, storeSettingsApi, type StoreCompanySettings, type StoreSocialLink } from '../services/api';
...
export function StoreLayout({ children, overlayHeader }: StoreLayoutProps) {
  ...
  const [company, setCompany] = useState<StoreCompanySettings>({
  ...
  useEffect(() => {
    ...
    storeSettingsApi.company().then(setCompany).catch(() => {});
  }, []);
  ...
  const location = [company.address, company.city, company.country].filter(Boolean).join(', ');
